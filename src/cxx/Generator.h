...
/**
 * Anything that can be generated by a `cxx::Generator`.
 * There's some redundancy here, but the concept is spelled out in this way
 * in the hopes that error messages list these out and provide some hints to the user.
 */
template <typename T>
concept Generatable =
        std::semiregular<T>               // `semiregular`s work fine (overlaps with below)
        || std::is_copy_assignable_v<T>   // `Generators` co_yield values by copying
        || std::is_base_of_v<RefBase, T>  // `Ref`'s also work, as a last resort
        ;
...
static_assert(Generatable<Generator<int>>);
